...
for i in range(ITERATIONS + 1):
    ...
    for batch_number in range(len(batched_x)):
        batched_activations = forward_propogation(batched_x[batch_number], weights)
        weights = back_propogation(batched_activations, weights, batched_y[batch_number], effective_lr)
        MSE_List.append(calculate_error(batched_activations[-1], batched_y[batch_number]))
    # STOPPING CRITERIA
