...
def initialize_weights():
    LAYERS = [FEATURES] + \
        HIDDEN_LAYER_ARCH + [TARGET_CLASSES]
    W = []
    for i in range(len(LAYERS)-1):
        epsilon = math.sqrt((LAYERS[i]+LAYERS[i+1])/1000)
        W_temp = np.random.uniform(-1*epsilon, epsilon,
...
NUMBER_OF_HIDDEN_UNITS_LIST = [5, 10, 15, 20, 25]
trainAccuracyList = []
testAccuracyList = []
timeTakenList = []
...
for x in NUMBER_OF_HIDDEN_UNITS_LIST:

    HIDDEN_LAYER_ARCH = [x]
    print("Running for", x, "hidden units")
    ...
    trainAccuracy = round(calc_accuracy(output, encoded_y), 2)
    timeTakenList.append(timeTaken)
    trainAccuracyList.append(trainAccuracy)
    print(f'Training Time : {timeTaken}  secs')
    ...
    testAccuracy = round(calc_accuracy(test_output, y_test), 2)
...
plt.plot(NUMBER_OF_HIDDEN_UNITS_LIST, trainAccuracyList)
plt.xlabel("Number of Hidden Units")
plt.ylabel("Training Accuracy")
plt.title("Training Accuracy v/s Number of Hidden Units")
plt.grid(b=True, which='major', color='#666666', linestyle='-', alpha=0.5)
plt.show()
plt.close()

plt.plot(NUMBER_OF_HIDDEN_UNITS_LIST, testAccuracyList)
plt.xlabel("Number of Hidden Units")
plt.ylabel("Testing Accuracy")
plt.title("Testing Accuracy v/s Number of Hidden Units")
plt.grid(b=True, which='major', color='#666666', linestyle='-', alpha=0.5)
plt.show()
plt.close()

plt.plot(NUMBER_OF_HIDDEN_UNITS_LIST, timeTakenList)
plt.xlabel("Number of Hidden Units")
plt.ylabel("Time Taken (in secs)")
plt.title("Time Taken v/s Number of Hidden Units")
plt.grid(b=True, which='major', color='#666666', linestyle='-', alpha=0.5)
plt.show()
plt.close()
